...
import { Dumbbell, Flame, CalendarDays, Check, Download, Upload, TrendingUp } from 'lucide-react';
...
export function DashboardPage() {
    ...
    if (selectedExercise && exerciseNames.includes(selectedExercise)) {
      const history = dayWorkouts
        ...
        .flatMap(w => w.exercises
          .filter((e: any) => e.name === selectedExercise && e.weights && e.weights.length > 0)
          .map((e: any) => ({
            date: w.date,
            weight: Math.round(e.weights[e.weights.length - 1]),
          }))
        )
  ...
  }, [dayWorkouts, routines, selectedExercise]);
    ...
    <div className="flex flex-col gap-6 p-4 pb-24">
      ...
      <div className="mt-4 pt-4 border-t border-border-subtle">
        ...
        <div className="flex gap-3">
          <PillButton variant="secondary" onClick={exportData} className="flex-1">
            <Download className="w-4 h-4 mr-2" />Exportar
          </PillButton>
          <PillButton variant="secondary" onClick={() => setShowImportModal(true)} className="flex-1">
      ...
      {showImportModal && (
        <div className="fixed inset-0 bg-bg-deep/80 z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-sm bg-bg-surface rounded-card p-6 border border-border-subtle">
